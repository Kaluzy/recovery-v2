...
function render(){
 ...
 const mins=state.training.reduce((n,x)=>n+(+x.mins||0),0),days=Math.max(1,new Set(state.meals.map(m=>m.day)).size);$('#trendStats').innerHTML='<div class="mini"><b>~'+Math.round(all.kcal/days)+'</b><span class="sub">kcal/day</span></div><div class="mini"><b>~'+Math.round(all.protein/days)+'g</b><span class="sub">protein/day</span></div><div class="mini"><b>'+mins+'</b><span class="sub">training min</span></div>';$('#notes').value=state.notes||'';document.querySelectorAll('[data-floor]').forEach(b=>b.classList.toggle('done',!!state.floor[b.dataset.floor]));
}
